...
export function Journey() {
  const experiences = data.experiences.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  return (
    <section id="experience" className="py-24 sm:py-32 overflow-hidden bg-background/80">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-4xl font-bold tracking-tight text-foreground font-headline sm:text-5xl">{strings.experience.title}</h2>
          <p className="mt-4 text-lg text-muted-foreground">{strings.experience.subtitle}</p>
        </motion.div>

        {/* Experience Timeline */}
        <div className="relative max-w-2xl mx-auto">
          <div className="absolute left-4 top-0 h-full w-0.5 bg-border/50 -z-10"></div>
