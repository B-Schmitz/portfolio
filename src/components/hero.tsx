    ...
    <section id="about" className="relative min-h-screen flex items-center justify-center overflow-hidden bg-background">
      ...
      >
        <motion.div
          ...
          variants={containerVariants}
        >
          <motion.h1 className="text-4xl md:text-6xl font-bold text-foreground font-headline" variants={itemVariants}>
            {strings.hero.name}
          </motion.h1>
          <motion.p className="mt-4 text-xl md:text-2xl text-primary" variants={itemVariants}>
            {strings.hero.title}
          </motion.p>
          <motion.p className="mt-6 text-base md:text-lg text-muted-foreground" variants={itemVariants}>
            {strings.hero.bio}
          </motion.p>
          <motion.div className="mt-8 flex justify-center md:justify-start gap-4" variants={itemVariants}>
            ...
            <Button size="lg" variant="outline" asChild>
              <a href={"https://drive.google.com/file/d/1hykLXSWL3SRtCLIEnup0ZEGhyYwY7BZO/view"} target='_blank'><LinkIcon className="mr-2 h-4 w-4" /> {strings.contact.button}</a>
            </Button>
          </motion.div>
        </motion.div>
      </motion.div>
